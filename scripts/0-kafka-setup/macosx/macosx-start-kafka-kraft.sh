...
# example
# log.dirs=$HOME/apache-kafka/data/kafka-kraft
...
kafka-storage.sh format -t <uuid> -c ~/apache-kafka/kafka_2.13-3.7.0/config/kraft/server.properties

# This will format the directory that is in the log.dirs in the config/kraft/server.properties file

# start Kafka
kafka-server-start.sh ~/apache-kafka/kafka_2.13-3.7.0/config/kraft/server.properties
